...
def assign_water_hydrogens(
    ...
    if o_indices.size != o_count:
        raise ValueError(f"Found {o_indices.size} O atoms, but POSCAR lists {o_count}.")
    if h_indices.size != h_count:
        raise ValueError(f"Found {h_indices.size} H atoms, but POSCAR lists {h_count}.")

    delta = coords[o_indices, None, :] - coords[h_indices][None, :, :]
    dist = vector_norm(minimum_image(delta, cell, inv_cell))

    candidates: list[tuple[float, int, int]] = []
    for oi in range(o_indices.size):
        for hi in np.argsort(dist[oi])[:8]:
            if dist[oi, hi] <= covalent_cutoff:
                candidates.append((float(dist[oi, hi]), oi, int(hi)))
    candidates.sort()

    assigned_h: list[list[int]] = [[] for _ in range(o_indices.size)]
    used_h: set[int] = set()
    for _distance, oi, hi in candidates:
        if hi in used_h or len(assigned_h[oi]) >= 2:
            continue
        assigned_h[oi].append(int(h_indices[hi]))
        used_h.add(hi)

    water_rows = [i for i, hs in enumerate(assigned_h) if len(hs) == 2]
    if len(water_rows) != expected_waters:
        missing = [int(o_indices[i]) + 1 for i, hs in enumerate(assigned_h) if len(hs) != 2]
        raise ValueError(
            "Could not identify the expected number of waters from unique O-H "
            f"pairs with cutoff {covalent_cutoff} A: expected {expected_waters}, "
            f"identified {len(water_rows)}. Non-water/ambiguous O serials begin: {missing[:20]}"
        )

    water_o = o_indices[water_rows]
    water_h = np.array([assigned_h[i] for i in water_rows], dtype=np.int64)
...
def main() -> None:
    ...
    parser.add_argument("--expected-waters", default=100, type=int)
    ...
    water_o, water_h = assign_water_hydrogens(
        ...
        o_count,
        h_count,
        args.covalent_oh_cutoff,
        args.expected_waters,
    )
    nwater = water_o.size
    inv_cell = np.linalg.inv(cell)
