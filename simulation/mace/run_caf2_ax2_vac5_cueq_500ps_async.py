#!/usr/bin/env python3
"""Restartable all-atom CaF2/oil/water MACE NVT MD with compressed output."""
...
def arguments():
    ...
    p.add_argument("--audit-interval-steps", type=int, default=200,
                   help="full O(N^2) geometry audit interval; fast finite checks stay every frame")
    p.add_argument("--compressor", default=None,
                   help="background HDF5 compressor script")
    return p.parse_args()
...
class Audit:
    ...
    def update(self, atoms, absolute_step, full_geometry=True):
        if len(atoms) != len(self.expected_numbers) or not np.array_equal(atoms.numbers, self.expected_numbers):
...
def create_h5(path, nframes, atoms, segment_index, first_abs_step, compressed=False):
    h = h5py.File(path, "w", libver="latest")
    c = min(128, nframes)
    common = {"compression": "gzip", "compression_opts": 1, "shuffle": True} if compressed else {}
    h.create_dataset("step", (nframes,), dtype="i8", chunks=(c,), **common)
    h.create_dataset("time_ps", (nframes,), dtype="f8", chunks=(c,), **common)
    h.create_dataset("positions_A", (nframes, len(atoms), 3), dtype="f4",
                     chunks=(c, len(atoms), 3), **common)
    ...
    h.attrs["all_atoms_mobile"] = True
    h.attrs["live_compression"] = compressed
    h.attrs["mace_cueq_enabled"] = True
    return h
...
def main():
    ...
    compressor = Path(args.compressor).resolve() if args.compressor else Path(__file__).with_name("compress_caf2_h5_segment.py")
    if not compressor.exists():
        raise SystemExit(f"Compressor script not found: {compressor}")
    atoms = read(source)
    source_contains_momenta = atoms.has("momenta")
    if not source_contains_momenta:
    ...
    frames_done = 0
    thermostat_origin = "new_chain_from_original_structure"
    ...
    # A previous async run may have completed a raw segment just before a stop.
    # Finish those conversions synchronously before trusting the checkpoint.
    for raw in sorted(segments.glob("segment_*.raw.h5")):
        final = segments / raw.name.replace(".raw.h5", ".h5")
        if not final.exists():
            subprocess.run([sys.executable, str(compressor), str(raw), str(final)], check=True)
    ...
    metadata = {
        ...
        "source_step": SOURCE_STEP, "source_time_ps": 0.0,
        "source_contains_momenta": source_contains_momenta,
        "source_temperature_K": source_temperature,
        "velocity_initialization": (
            "preserved_from_source" if source_contains_momenta else
            f"MaxwellBoltzmann 300 K, Stationary, seed={args.seed}; all atoms mobile"
        ),
        "thermostat_state_note": (
            "Production begins from the earliest conf.lmp-derived a*2, c+5 A structure. "
            "A new 300 K Nose-Hoover chain starts at zero; segment restarts restore eta/p_eta."
        ),
        ...
        "structure_transform": "original conf.lmp repeated (2,1,1), c vector extended by 5 A without scaling atoms",
        "constraints": "none", "all_atoms_mobile": True,
        "mobile_atom_count": len(atoms),
        "mace_acceleration": "cuEquivariance enabled", "seed": args.seed,
        "full_geometry_audit_interval_steps": args.audit_interval_steps,
        "fast_checks": "finite energy temperature force every output frame",
        "compression_mode": "uncompressed raw HDF5 on MD path; gzip-1 conversion in background process",
        "datasets": {
    ...
    try:
        while continuation_steps < target_steps:
            ...
            partial = segments / f"{base}.raw.partial.h5"
            raw = segments / f"{base}.raw.h5"
            final = segments / f"{base}.h5"
            h = create_h5(partial, nframes, atoms, segment_index, first_abs, compressed=False)
            try:
                for i in range(nframes):
                    dyn.run(OUTPUT_EVERY_STEPS)
                    continuation_steps += OUTPUT_EVERY_STEPS
                    absolute_step = SOURCE_STEP + continuation_steps
                    full_geometry = (i == 0 or absolute_step % args.audit_interval_steps == 0)
                    pe, temp, fmax = audit.update(atoms, absolute_step, full_geometry=full_geometry)
            ...
            log_path = output / "compression_async.log"
            log_handle = log_path.open("a")
            proc = subprocess.Popen(
                [sys.executable, str(compressor), str(raw), str(final)],
                stdout=log_handle, stderr=subprocess.STDOUT,
            )
            compression_jobs.append((proc, log_handle, raw, final))
            record = {
                "segment_index": segment_index, "file": str(final),
                "sha256": None, "compression": "pending", "frames": nframes,
            ...
            progress["compression_pending_segments"] = sum(p.poll() is None for p, *_ in compression_jobs)
            atomic_json(output / "progress.json", progress)
            atomic_json(audit_checkpoint, progress)
        for proc, log_handle, raw, final in compression_jobs:
            rc = proc.wait()
            log_handle.close()
            if rc != 0:
                raise RuntimeError(f"Async compression failed for {raw} (exit {rc})")
        manifest_path = output / "segments_manifest.json"
        records = json.loads(manifest_path.read_text()) if manifest_path.exists() else []
        existing = {r.get("segment_index") for r in records}
        for record in new_records:
            if record["segment_index"] not in existing:
                record["sha256"] = sha256(Path(record["file"]))
                record["compression"] = "complete"
